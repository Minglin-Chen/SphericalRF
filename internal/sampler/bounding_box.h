...
#include "internal/sampler/triangle.h"


template <int N_POINTS>
HOST_DEVICE inline void project(Eigen::Vector3f points[N_POINTS], const Eigen::Vector3f& axis, float& min, float& max) {
...
struct BoundingBox {
	...
	// From triangle
	HOST_DEVICE explicit BoundingBox(const Triangle& tri) {
		min = max = tri.a;
		enlarge(tri.b);
		enlarge(tri.c);
	}
	...
	HOST_DEVICE void enlarge(const Triangle& tri) {
		enlarge(tri.a);
		enlarge(tri.b);
		enlarge(tri.c);
	}
	...
	// HOST_DEVICE bool intersects(const Triangle& triangle) const {
	...
	HOST_DEVICE float distance(const Eigen::Vector3f& p) const {
		return sqrt(distance_sq(p));
	}

	HOST_DEVICE float distance_sq(const Eigen::Vector3f& p) const {
		return (min - p).cwiseMax(p - max).cwiseMax(0.0f).squaredNorm();
	}

	HOST_DEVICE float signed_distance(const Eigen::Vector3f& p) const {
		Eigen::Vector3f q = (p - min).cwiseAbs() - diag();
		return q.cwiseMax(0.0f).norm() + std::min(std::max(q.x(), std::max(q.y(), q.z())), 0.0f);
	}

	HOST_DEVICE void get_vertices(Eigen::Vector3f v[8]) const {
