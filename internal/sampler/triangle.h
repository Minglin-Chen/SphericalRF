...
struct Triangle {
	HOST_DEVICE Eigen::Vector3f sample_uniform_position(const Eigen::Vector2f& sample) const {
	...
	HOST_DEVICE Eigen::Vector3f normal() const {
		return (b - a).cross(c - a).normalized();
	}

	HOST_DEVICE float ray_intersect(const Eigen::Vector3f &ro, const Eigen::Vector3f &rd, Eigen::Vector3f& n) const { // based on https://www.iquilezles.org/www/articles/intersectors/intersectors.htm
	...
	HOST_DEVICE float ray_intersect(const Eigen::Vector3f &ro, const Eigen::Vector3f &rd) const {
		Eigen::Vector3f n;
		return ray_intersect(ro, rd, n);
	}

	HOST_DEVICE float distance_sq(const Eigen::Vector3f& pos) const {
		// prepare data
		...
		return
			// inside/outside test
			(copysignf(1.f, v21.cross(nor).dot(p1)) + copysignf(1.f, v32.cross(nor).dot(p2)) + copysignf(1.f, v13.cross(nor).dot(p3)) < 2.0f)
			?
			// 3 edges
			std::min(
				std::min(
	...
	HOST_DEVICE float distance(const Eigen::Vector3f& pos) const {
		return std::sqrt(distance_sq(pos));
	}

	HOST_DEVICE bool point_in_triangle(const Eigen::Vector3f& p) const {
	...
	HOST_DEVICE Eigen::Vector3f closest_point_to_line(const Eigen::Vector3f& a, const Eigen::Vector3f& b, const Eigen::Vector3f& c) const {
		float t = (c - a).dot(b-a) / (b-a).dot(b-a);
		t = std::max(std::min(t, 1.0f), 0.0f);
		return a + t * (b - a);
	}

	HOST_DEVICE Eigen::Vector3f closest_point(Eigen::Vector3f point) const {
	...
	HOST_DEVICE Eigen::Vector3f centroid() const {
		return (a + b + c) / 3.0f;
	}

	HOST_DEVICE float centroid(int axis) const {
		return (a[axis] + b[axis] + c[axis]) / 3;
	}

	HOST_DEVICE void get_vertices(Eigen::Vector3f v[3]) const {
		v[0] = a;
		v[1] = b;
		v[2] = c;
	}
